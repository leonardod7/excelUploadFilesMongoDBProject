...
@app.callback(
    [
        ...
        Output("progress-container", "style"),  # Controla a visibilidade da barra
    ],
    [
    ...
    if triggered_id == "btn-processar":
        ...
        if value is None:
            return (
                ...
                True,
                0,
                "Selecione uma opção no dropdown antes de processar.",
                {"display": "none"},  # Esconde a barra se não for válida
            )

        # Reinicia a barra de progresso e a exibe
        time.sleep(1)  # Simula um pequeno atraso inicial
        return 0, "0%", False, 0, dash.no_update, {"display": "block"}
        ...
        if progress >= 100:
            return (
                ...
                {"display": "none"},  # Esconde a barra ao finalizar
            )
        ...
        return (
            ...
            dash.no_update,
            dash.no_update,
            {"display": "block"},  # Garante que a barra continue visível
        )
